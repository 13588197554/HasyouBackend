package com.fly.config;

/**
 * @author david
 * @date 22/09/18 13:21
 */
public class Constants {

    public static final String BASE_MOVIE_URL = "https://api.douban.com/v2/movie/subject/";

    public static final String API_KEY = "0b2bdeda43b5688921839c8ecb20399b";

    public static final String MOVIE_KEY = "MOVIE_JOB";

    public static final String MOVIE_EXP_KEY = "MOVIE_EXP_KEY";

    public static final String BOOK_INFO_KEY = "BOOK_INFO_KEY";

    public static final String EXP_BOOK_INFO_KEY = "EXP_BOOK_INFO_KEY";

    public static final String BASE_BOOK_URL = "https://api.douban.com/v2/book/";

    public static final String BOOK_KEY = "BOOK_KEY";

    public static final String BOOK_EXP_KEY = "BOOK_EXP_KEY";

    public static final String BOOK_COMMENT_KEY = "BOOK_COMMENT_KEY";

    public static final String EXP_BOOK_COMMENT_KEY = "EXP_BOOK_COMMENT_KEY";

    /* v2 url */
    public static final String V2_HOT_URL = "https://www.v2ex.com/api/topics/hot.json";

    public static final String V2_LATEST_URL = "https://www.v2ex.com/api/topics/latest.json";

}
